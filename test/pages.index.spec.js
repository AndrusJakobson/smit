...
describe('index.vue', () => {
  ...
  it('should console.log on button click', async () => {
    await wrapper.vm.getBooks();
    console.log = jest.fn();

    expect(wrapper.find('.v-data-table__wrapper button').trigger('click'));
    expect(console.log).toHaveBeenLastCalledWith('Osta: TestHeadline');
  });
});
